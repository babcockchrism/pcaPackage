...
#' @examples
#' p <- pca(iris)
#' cluster_analysis(p$pca, num_clusters = NA, max_clusters = 10, x = "PC1", y = "PC2", nstart = 10)
cluster_analysis <- function(pca, num_clusters = NA, max_clusters = 10, x = "PC1", y = "PC2", nstart = 10) {

  # Compute TWSS
  twss <- c()
  for(i in 1:max_clusters) {
    twss <- append(twss, stats::kmeans(pca, centers = i, nstart = nstart)$tot.withinss)
  }
  ...
  # Final clustering
  kmeans_result <- stats::kmeans(pca, centers = num_clusters, nstart = nstart)
